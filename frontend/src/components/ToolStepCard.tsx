...
export function ToolStepCard({ step }: { step: ToolStep }) {
  ...
  const language = (step.args.language as string) || "python";
  const label =
    step.tool === "execute_code"
      ? `Running ${language.charAt(0).toUpperCase()}${language.slice(1)}`
  ...
  const preview =
    step.tool === "web_search"
      ...
      : (step.args.code as string)?.split("\n")[0] || "Code snippet";
        ...
        <div className="px-3 py-2 space-y-2 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-700">
          {step.tool === "execute_code" ? (
            <div>
              ...
              <pre className="text-xs text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap break-all bg-zinc-50 dark:bg-zinc-800 rounded p-2 max-h-60 overflow-y-auto">
